...
class TestHeaderFieldRange:
    ...
    def test_range_property_basic(self) -> None:
        ...
        assert start == 10
        assert stop == 14  # INT32 is 4 bytes, so 10 + 4 = 14
    ...
    def test_range_property_different_types(self, format_type: ScalarType, expected_size: int) -> None:
        """Test range property with different data types."""
        field = HeaderField(name="field", format=format_type, byte=100)
        start, stop, name = field.range
        
        assert start == 100
        assert stop == 100 + expected_size
        assert name == "field"
    ...
    def test_range_property_different_byte_positions(self) -> None:
        ...
        for byte_pos in test_cases:
            ...
            assert stop == byte_pos + 2  # INT16 is 2 bytes
            assert name == "test"
...
class TestMergeHeadersByName:
    ...
    def test_merge_with_name_conflicts(self, segy_spec: SegySpec) -> None:
        ...
        new_fields = [
            HeaderField(name="field1", format=ScalarType.FLOAT32, byte=100),  # Replaces existing field1
            HeaderField(name="new_field", format=ScalarType.INT16, byte=200),  # New field
        ]
        
        result = segy_spec._merge_headers_by_name(existing_fields, new_fields)
        
        assert len(result) == 4  # 2 new fields + 2 non-conflicting existing fields
        
        # New fields come first
        assert result[0].name == "field1"
        assert result[0].format == ScalarType.FLOAT32
        assert result[0].byte == 100
...
class TestValidateNonOverlappingHeaders:
    ...
    def test_validate_overlapping_fields_raises_error(self, segy_spec: SegySpec) -> None:
    ...
    def test_validate_multiple_overlaps_raises_error(self, segy_spec: SegySpec) -> None:
        ...
        fields = [
            HeaderField(name="field1", format=ScalarType.INT32, byte=1),   # bytes 1-4
            HeaderField(name="field2", format=ScalarType.INT32, byte=3),   # bytes 3-6 (overlaps with field1)
            HeaderField(name="field3", format=ScalarType.INT16, byte=5),   # bytes 5-6 (overlaps with field2)
        ]
...
class TestMergeHeadersByByteOffset:
    ...
    def test_merge_by_offset_no_overlaps(self, segy_spec: SegySpec) -> None:
        ...
        existing_fields = [
            ...
            HeaderField(name="existing2", format=ScalarType.INT16, byte=10),  # bytes 10-11
        ]
        new_fields = [
    ...
    def test_merge_by_offset_with_overlaps(self, segy_spec: SegySpec) -> None:
        ...
        existing_fields = [
            ...
            HeaderField(name="existing3", format=ScalarType.INT16, byte=20),  # bytes 20-21
        ]
        new_fields = [
            HeaderField(name="new1", format=ScalarType.UINT32, byte=2),       # bytes 2-5 (overlaps with existing1)
        ]
        ...
        result = segy_spec._merge_headers_by_byte_offset(existing_fields.copy(), new_fields)
    ...
    def test_merge_by_offset_multiple_overlaps(self, segy_spec: SegySpec) -> None:
        ...
        existing_fields = [
            ...
            HeaderField(name="existing3", format=ScalarType.INT32, byte=15),  # bytes 15-18
        ]
        new_fields = [
            HeaderField(name="new1", format=ScalarType.UINT32, byte=3),       # bytes 3-6 (overlaps existing1 & existing2)
            HeaderField(name="new2", format=ScalarType.UINT16, byte=20),      # bytes 20-21 (no overlap)
        ]
...
class TestCustomizeMethodEnhanced:
    ...
    def test_customize_binary_headers_with_duplicate_names_raises_error(self, segy_spec: SegySpec) -> None:
    ...
    def test_customize_trace_headers_with_duplicate_names_raises_error(self, segy_spec: SegySpec) -> None:
    ...
    def test_customize_binary_headers_with_overlapping_fields_raises_error(self, segy_spec: SegySpec) -> None:
    ...
    def test_customize_trace_headers_with_overlapping_fields_raises_error(self, segy_spec: SegySpec) -> None:
    ...
    def test_customize_replaces_existing_fields_by_name(self, segy_spec: SegySpec) -> None:
        ...
        assert replaced_field.byte == 300
    ...
    def test_customize_handles_none_inputs(self, segy_spec: SegySpec) -> None:
        ...
        custom_spec = segy_spec.customize(
            binary_header_fields=None,
            trace_header_fields=None
        )
